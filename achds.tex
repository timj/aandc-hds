...
make the extensible N-Dimensional Data Format for acessible to a
...
System before being rebrand as HDS, the first version of the library
...
none of the remaining developers understand how HDS works and there is
no motivation for anyone to learn. The most recent description of the
...
does not work. It is not clear how much work it would be to finish
...
inefficient for row access. Switching data formats raises the
possibility to add a native table access API to HDS.
...
adopt NDF currently requires that users adopt HDS. This is true for
all the Starlink applications which require the \textsc{convert}
package be used to migrate from FITS and other formats to NDF/HDS.
Adopting HDF5 would considerably lower the barrier to entry and make
...
name of the structure is an object instance.
...
access models diverge. The previous version of HDS was version 4 so it
was decided this version would be version 5\footnote{Somewhat
  confusingly the library implementing version 4 of the file format is
  itself version 5}. In the rest of this paper we use the shorthand
...
\old\ library to see if it opens. If that fails because with a format
...
\old\ format using the principal of least surprise. The ability to
...
sizeof of these \texttt{HDSLoc*} structures differs between the two
...
\old\ the C structure is really a proxy to an internal data
...
identifiers such that a file is only kept open when the primary count
hits zero even if secondary identifiers remain. To implement this in
...
implement a hash table indexed by the HDF5 file identifier where each
key corresponds to a value of a \texttt{utarray} dynamic array
containing the locators with each locator having a flag indicating
whether it is primary or secondary. The \texttt{uthash} macros were
...
An important requirement for any HDS implementation has been to
...
file), HDS had no issue with this but with this HDF5 strictly
...
Now that a functioning prototype exist and has been proven to work
...
for testing development releases of the new library.
