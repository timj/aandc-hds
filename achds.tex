...
make the extensible \emph{N}-Dimensional Data Format accessible to a
...
pipelines \citep{2011tfa..confE..42J,2015A&C.....9...40J,2014ASPC..485..143B}. Its presence was
...
\emph{N}-Dimensional Data Format library \citep[NDF;][\ascl{1411.023}]{2015Jenness}. There was
...
new APIs to allow both 32-bit and 64-bit application software to co-exist.
...
was decided that this version would be version 5.
...
standalone libraries that are linked into a wrapper library that
...
size of these structures differs between the two
...
created and within it are placed a number of groups corresponding to
...
\caption{Comparison of the HDS view of an HDF5 data file with the HDF5
  view.
  (a) Listing of an HDSv5 file using the standard HDS tracing tool,
...
attempt to guess what the HDF5 library has chosen to do by normalizing
...
the image displayed within 2\,s and the process only used a
fews tens of megabytes of memory. With memory mapping disabled it took
about 10\,s to load the image and the process took 5\,GB of
memory.
...
\begin{table*}[ht]
  \caption{A comparison of file sizes resulting from identical
    ...
    datasets. All files sizes are in bytes.}
\label{tab:size}
\begin{center}
...
the original file sizes.\footnote{All these tests were done using the
  ...
  parameter file test. It has not yet been decided whether \new\ should
...
\item The AGI graphics database generated by the \textsc{SpecDRE}
  demonstration script \citep[][\ascl{1407.003}]{1992StarB..10....8M}. The graphics database makes
...
\item \KAPPA\ \citep[][\ascl{1403.022}]{SUN95} logo image consisting of a simple NDF
...
provide the prospect of native table access (the import of FITS binary tables would
...
I thank the two referees for their comments on the paper and also
their comments on how to improve the library.
